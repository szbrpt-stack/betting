...
# BOOTSTRAP: Asegurar datos reales al arrancar en la nube
with engine.connect() as conn:
    try:
        # Intentar contar partidos, si falla o es 0, ejecutamos seed
        res = conn.execute(text("SELECT COUNT(*) FROM matches")).scalar()
        if res == 0:
            logger.info("Base de datos vacía en Cloud. Poblando...")
            harvester.seed_data()
    except Exception as e:
        logger.warning(f"Error en chequeo inicial: {e}. Inicializando DB...")
        harvester.init_db()
        harvester.seed_data()
...
async def root():
    return {
        "status": "PropsBR Meta-Aggregator Online",
        "version": "1.5.0",
        "endpoints": ["/api/v1/full-catalog", "/api/v1/sync"]
    }
